function I_SectionPlot_rotated(a,transect,transectname) 
%===================================================
% Script Name: I_SectionPlot_rotated2.m
%
...
%				Input currents were aligned along the  transect
%				
...
for kk=1:length(transect) % create an array for the 
        ...
        if HUDLADCP(transect_index(kk),1).ualign~=0
          u=  HUDLADCP(transect_index(kk),1).ualign;
          v=  HUDLADCP(transect_index(kk),1).valign;
...
zmin=-.6; zmax=0.6; %limits of current velocity for colour scale %dz=0.04;
   ttlEW = [['HUD', a.cruise_id], ', E W Current Speed, Aligned with Transect']; %titles
   ttlNS = [['HUD', a.cruise_id], ', N S Current Speed, Aligned with Transect']; 
...
%    distlon = pos2dist(latloncastsort(end,1),latloncastsort(end,2),latloncastsort(1,1),latloncastsort(1,2),1); %convert Lat, lon locations to distance
%    distlonmax = distlon %+distlon/4; %maximum value for x-axis
%    xi = 0:1:distlonmax; % x scale 
...
%----------------------------------------------------------------------
 ...
 figure(1)   
 filename=[['HUD', a.cruise_id], 'rotated_USection_', transectname];
 print('-djpeg', '-r300', filename);
 hgsave(filename);
 
figure(2)
 filename=[['HUD', a.cruise_id], 'rotated_VSection_',transectname ];
 print('-djpeg', '-r300', filename);
 hgsave(filename);
