...
function topo1min = m_plot_topo1min_noland2(figtitle,latmax,latmin,lonmax,lonmin,lat_points,lon_points,slist,heading)
%input
% figtitle=input('enter name for map')
% latmax=input('enter maximum Latitude')
% latmin=input('enter minimum Latitude')  
% lonmax=input('enter maximum Longitude')
% lonmin=input('enter minimum Longitude') 
% lat_points=input('enter Latitude of points to plot') 
% lon_points=input('enter Longitude of points to plot (use neg for west)') 
...
[cs h]=m_contourf(lon,lat,elevations,[-100 -200 -300 -400 -500 -1000 -1500 -2000 -2500 -3000 -4000]);
...
m_plot(lon_points(:),lat_points(:), 'ro','MarkerEdgeColor','r','MarkerFaceColor','r','MarkerSize',2) % plots given point on map 
   ...
   for s = 1:length(heading)
       if isnan(heading(s,1))~=1
%        linedirx=[heading(s,1) , heading(s,2)];  linediry=[heading(s,3) , heading(s,4)];
      m_plot([heading(s,1)  heading(s,2)], [heading(s,3)  heading(s,4)],'color',[.5 .5 .5])
%       m_plot(linedirx, linediry ,'m')    
      m_plot(heading(s,1), heading(s,3) ,'marker','.','markersize',6,'color',[.5 .5 .5])     
%       m_plot(heading(s,2), heading(s,4) ,'kx')     
       end 
   end
title(figtitle)   
