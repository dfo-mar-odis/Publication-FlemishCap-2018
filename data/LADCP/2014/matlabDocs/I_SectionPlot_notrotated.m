function I_SectionPlot_notrotated(a,transect,transectname) 
%===================================================
% Script Name: I_SectionPlot_notrotated.m
%
...
%				Input currents were not aligned 
%				
...
for kk=1:length(transect) % create an array for the 
        ...
        if HUDLADCP(transect_index(kk),1).u~=0
          u=  HUDLADCP(transect_index(kk),1).u;
          v=  HUDLADCP(transect_index(kk),1).v;
...
zmin=-.6; zmax=0.6; %limits of current velocity for colour scale %dz=0.04;
   ttlEW = [['HUD', a.cruise_id], ', E W Current Speed, NOT Aligned']; %titles
   ttlNS = [['HUD', a.cruise_id], ', N S Current Speed, NOT Aligned']; 
...
%----------------------------------------------------------------------
  ...
  figure (uv)
   ...
   %Plot bathymetry profile
      ...
      Y = [yii,ymax*-1,ymax*-1,yii(1)];
      Xi  = X(find(~isnan(Y)));
      Yi  = Y(find(~isnan(Y)));
      fill(Xi,Yi*-1,[0.7,0.7,0.7]), hold on;
...
%----------------------------------------------------------------------
 ...
 figure(1)   
 filename=[['HUD', a.cruise_id], 'USectionbathy_', transectname];
 print('-djpeg', '-r300', filename);
 hgsave(filename);
 
figure(2)
 filename=[['HUD', a.cruise_id], 'VSectionbathy_',transectname ];
 print('-djpeg', '-r300', filename);
 hgsave(filename);
