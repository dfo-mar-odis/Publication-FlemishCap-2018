...
m20u <- try(nls(uvar ~ sigma02 * cosh((kappa * N * depth)/f)^2, start = list(sigma02 = 5e-04, kappa = 2*pi/100/1000)))
...
uvarg5dFC <- uvarg5d
uvarl5dFC <- uvarl5d
uvarsumFC <- uvarg5dFC + uvarl5dFC
uvarFC <- uvar
pmFC <- predict(m20u)
depthFC <- depth

save(uvarFC, uvarsumFC, pmFC, depthFC, file='varianceFC.rda')

kappauFC <- kappau
kappauseFC <- kappause
kappaunFC <- kappaun
save(kappauFC, kappauseFC, kappaunFC, file='kappaFC.rda')
