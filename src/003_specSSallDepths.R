...
if(!interactive()) png('003_specSS.png', width=7.5, height=3, res=200, units='in', pointsize=10)
spans <- c(5,3)
...
for (i in 1:length(data)){
	...
	if(i == 1){
		...
		mtext("Frequency (cph)", side=1, line=2.5)
	...
	if(i == length(data)){
		...
		lines(s$freq[1] + c(-.5,.5) * s$bandwidth, rep(.5, 2), col='black', lwd=1.5)
...
mtext(expression(paste('Variance (m'^2,'s'^-2,'cph'^-1,')')), side=1, line=2.5)
mtext('Depth ( m )', side=2, line=2)

lm <- lm(spec21 ~ depth)
dz <- mean(diff(depth))
slope <- coef(lm)[2] * dz
save(slope, file='linearSlopePeakSS.rda')
if(!interactive()) dev.off()

spans <- c(7,3)
if(!interactive()) png('003_specSSerrorBar.png', width=7.5, height=3, res=200, units='in', pointsize=10)
